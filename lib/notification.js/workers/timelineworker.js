...
var TimelineWorker = function (queue) {
	//console.log("TimelineWorker (constructor)");
	this._queue = queue;
	this._timer = null;
...
TimelineWorker.prototype._alarm = function (nowTS, dt) {
	...
	var jobstream = new JobStream(1, function (items, continue_callback) {
		...
		items = items.pop();
		var parsedItems = [];
		//console.log("checking items: "+items.length);
		for (var i in items) {
			var items_i = items[i];
	...
	//console.log("CALLING scanTimelineWithStream!");
	if (!isRunning) {
		isRunning = true;
...
TimelineWorker.prototype._prepareTimelineJobs = function (items, callback) {
	//console.log("prepareTimelineJobs");
...
TimelineWorker.prototype._prepareTimelineJob = function (item, callback) {
	//console.log("prepareTimelineJob");
	var doEnqueue = true;
...
function scanTimelineWithStream(table, inf, sup, limit, jobstream, callback, ExclusiveStartKey) {
	//console.log("scanTimelineWithStream");
	//console.log(table+":"+inf+":"+sup+":"+limit+":"+ExclusiveStartKey);
	var params = {
		ScanFilter:{id:{
